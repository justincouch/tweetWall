...
function connectToSocket(){
    ...
    socket.on('tweet', function(data){
        ...
        tweetHistory.push(data);
        addTweetDiv(data);
        //console.log(data);
        //tweetWords = tweetWords.concat(data.text, " ");
        //refreshTweetQueue();
...
function checkTweetQueue(){
    if ( tweetQueue.length != 0 ){
        var lc = $("#left-col");
        var tq0 = $( "#t_"+tweetQueue[0].id_str );
        //$( tq0, lc ).remove();
        var tq02 = tq0.clone().appendTo($("body"));
        ...
        var tweetToAdd = tweetQueue.shift();
        tweetWords = tweetWords.concat((tweetToAdd.full_text != undefined ? tweetToAdd.full_text : tweetToAdd.text), " ");
        //parseText(tweetWords);
    }
    else {
...
function addTweetDiv(data){
    var lc = $("#left-col");
    lc.append( createTweetDiv(data) );
}

function refreshTweetQueue(){
    // var lc = $("#left-col");
    // //lc.empty();
    // var tweetDiv;
    // for ( var i=0; i<tweetQueue.length; i++ ){
    //     tweetDiv = createTweetDiv(tweetQueue[i]);
    //     lc.append(tweetDiv);
    // }
}

function createTweetDiv(t){
    var divString = '';
...
function draw(t, e) {
    ...
    vis.transition().delay(1e3).duration(750).attr("transform", "translate(" + [w >> 1, h >> 1] + ")scale(" + scale + ")").each("end", function(){
...
var unicodePunctuationRe = "!$%^&*-,-/:;?\\[-\\]_{}\xa1\xa7\xab\xb6\xb7\xbb\xbf\u037e\u0387\u055a-\u055f\u0589\u058a\u05be\u05c0\u05c3\u05c6\u05f3\u05f4\u0609\u060a\u060c\u060d\u061b\u061e\u061f\u066a-\u066d\u06d4\u0700-\u070d\u07f7-\u07f9\u0830-\u083e\u085e\u0964\u0965\u0970\u0af0\u0df4\u0e4f\u0e5a\u0e5b\u0f04-\u0f12\u0f14\u0f3a-\u0f3d\u0f85\u0fd0-\u0fd4\u0fd9\u0fda\u104a-\u104f\u10fb\u1360-\u1368\u1400\u166d\u166e\u169b\u169c\u16eb-\u16ed\u1735\u1736\u17d4-\u17d6\u17d8-\u17da\u1800-\u180a\u1944\u1945\u1a1e\u1a1f\u1aa0-\u1aa6\u1aa8-\u1aad\u1b5a-\u1b60\u1bfc-\u1bff\u1c3b-\u1c3f\u1c7e\u1c7f\u1cc0-\u1cc7\u1cd3\u2010-\u2027\u2030-\u2043\u2045-\u2051\u2053-\u205e\u207d\u207e\u208d\u208e\u2329\u232a\u2768-\u2775\u27c5\u27c6\u27e6-\u27ef\u2983-\u2998\u29d8-\u29db\u29fc\u29fd\u2cf9-\u2cfc\u2cfe\u2cff\u2d70\u2e00-\u2e2e\u2e30-\u2e3b\u3001-\u3003\u3008-\u3011\u3014-\u301f\u3030\u303d\u30a0\u30fb\ua4fe\ua4ff\ua60d-\ua60f\ua673\ua67e\ua6f2-\ua6f7\ua874-\ua877\ua8ce\ua8cf\ua8f8-\ua8fa\ua92e\ua92f\ua95f\ua9c1-\ua9cd\ua9de\ua9df\uaa5c-\uaa5f\uaade\uaadf\uaaf0\uaaf1\uabeb\ufd3e\ufd3f\ufe10-\ufe19\ufe30-\ufe52\ufe54-\ufe61\ufe63\ufe68\ufe6a\ufe6b\uff01-\uff03\uff05-\uff0a\uff0c-\uff0f\uff1a\uff1b\uff1f\uff20\uff3b-\uff3d\uff3f\uff5b\uff5d\uff5f-\uff65", fill = function(t,d){ var f = d3.scaleLinear().domain(d).range(["#ffffff", "#555555"]); console.log(f); return f; }/*d3.scale.category20b()*/, w = document.getElementById("center-col").offsetWidth, h = 680, words = [], max, scale = 1, complete = 0, keyword = "", tags, fontSize, maxLength = 30, fetcher, statusText = d3.select("#status"), layout = d3.layout.cloud().timeInterval(10).size([w, h]).fontSize(function(t) {
    return fontSize(+t.value)
}).text(function(t) {
    return t.key
}).on("word", progress).on("end", draw), svg = d3.select("#vis").append("svg").attr("width", w).attr("height", h), background = svg.append("g"), vis = svg.append("g").attr("transform", "translate(" + [w >> 1, h >> 1] + ")"); //echoForm = d3.select("body").append("form").attr("action", "https://www.jasondavies.com/echo").attr("target", "_blank").attr("method", "POST"), echoContentType = echoForm.append("input").attr("type", "hidden").attr("name", "content-type"), echoInput = echoForm.append("input").attr("type", "hidden").attr("name", "echo"), 
